...
function mainBlaster(fontFile:string, destinationFolder:string, userConfig:UserConf) {

    const config = Object.assign({}, defaultConfig, userConfig || {})
    const svgFontContent = fs.readFileSync(fontFile, 'utf-8');

    extractor(svgFontContent, config.filenames, function (characterSvgs:Array<IconInformation>) {
        ...
        const svgDir = path.resolve(process.cwd(), path.join(destinationFolder, 'svg'))
            const pngDir = path.resolve(process.cwd(), path.join(destinationFolder, 'png'))
